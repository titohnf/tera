      ...
      <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
        {navItems.map(item => {
          const isActive = item.href === '/tutor'
            ? pathname === '/tutor'
            : pathname.startsWith(item.href) || (item.alsoMatch && pathname.startsWith(item.alsoMatch))
          return (
            <Link
              key={item.href}
              href={item.href}
