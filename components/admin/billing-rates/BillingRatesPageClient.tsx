...
import BillingRatesManager, { type BillingRatesManagerHandle } from './BillingRatesManager'
import BillingRatesReadOnly from './BillingRatesReadOnly'
import {
  createBillingRatePeriod, setActiveBillingPeriod, deleteBillingRatePeriod,
  type BillingRate, type BillingRatePeriod,
} from '@/lib/actions/admin/billing-rates'

function formatDate(d: string) {
  return new Date(d).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })
}

export default function BillingRatesPageClient({
  periods,
  allRates,
}: {
  periods: BillingRatePeriod[]
  allRates: (BillingRate & { period_id: string })[]
}) {
  ...
  const managerRef = useRef<BillingRatesManagerHandle>(null)
  ...
  function handleCreatePeriod() {
    ...
    startTransition(async () => {
      const res = await createBillingRatePeriod(newName, newStart, newEnd || null, sourceId)
  ...
  function handleSetActive(periodId: string) {
    startTransition(async () => { await setActiveBillingPeriod(periodId) })
  }

  function handleDelete(periodId: string) {
    if (!confirm('Hapus periode ini beserta semua tarifnya?')) return
    startTransition(async () => {
      const res = await deleteBillingRatePeriod(periodId)
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Grup: harga per bulan · Privat: harga per pertemuan</p>
        </div>
        <button
                ...
                <div key={p.id} className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
                  ...
                  {isOpen && (
                    <div className="border-t border-slate-100">
                      ...
                      <BillingRatesReadOnly rates={rates} />
                    </div>
                  )}
                </div>
              )
