import type { BillingRate } from '@/lib/actions/admin/billing-rates'
...
export default function BillingRatesReadOnly({ rates }: { rates: BillingRate[] }) {
  const rateMap: Record<string, number> = {}
  for (const r of rates) {
    rateMap[`${r.class_type}|${r.jenjang}|${r.jenis}`] = r.amount
  }
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-300 bg-gray-50">
                ...
                <th className="text-right px-3 py-2 text-xs font-medium text-gray-400 w-36">{unit}</th>
              </tr>
            </thead>
