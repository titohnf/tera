...
export async function autoGenerateDraftPayslips(month: string) {
  ...
  const activeByTutor = new Map<string, string>()   // tutorId → payslipId (non-deleted)
  const deletedByTutor = new Map<string, string>()  // tutorId → payslipId (soft-deleted)
  for (const p of existingPayslips ?? []) {
    if (p.is_deleted) deletedByTutor.set(p.tutor_id, p.id)
    else activeByTutor.set(p.tutor_id, p.id)
  }

  // Tutor yang belum punya payslip aktif
  const tutorsMissing = tutorIds.filter(id => !activeByTutor.has(id))
  if (tutorsMissing.length === 0) return

  // Hanya hitung sesi completed untuk isi payslip
  ...
  for (const tutorId of tutorsMissing) {
    ...
    if (deletedId) {
      toRestore.push({ id: deletedId, data })
    } else {
      toInsert.push(data)
    }
  }
  ...
  await Promise.all([
    toInsert.length > 0
      ? supabase.from('payslips').insert(toInsert)
      : Promise.resolve(),
